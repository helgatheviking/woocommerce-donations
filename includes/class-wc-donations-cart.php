<?php
/**
 * WooCommerce Donations cart functions and filters.
 *
 * @class 	WC_Donations_Cart
 * @version 0.1.0
 * @since   0.1.0
 */
...
class WC_Donations_Cart {
	...
	public function is_purchasable( $purchasable , $product ) {
		if( WC_Donations_Helpers::is_product_checkbox( $product->id ) ){
			$purchasable = true;
		} else {
			$purchasable = false;
		}
	...
	public function add_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
		...
		if( WC_Donations_Helpers::is_product_checkbox( $product_id ) ){
			$cart_item_data['wc-boilerplate-extension-checkbox'] = 'true';
		}

		if( $number = WC_Donations_Helpers::get_sample_number( $product_id ) ){
			$cart_item_data['wc-boilerplate-extension-number'] = $number;
		}

		if( $number = WC_Donations_Helpers::get_sample_textbox( $product_id ) ){
	...
	public function add_cart_item( $cart_item ) {
		...
		if ( WC_Donations_Helpers::is_product_checkbox( $cart_item['product_id'] ) ) {
	...
	public function validate_add_cart_item( $passed, $product_id, $quantity, $variation_id = '', $variations= '' ) {
		...
		if ( WC_Donations_Helpers::is_product_checkbox( $product_id ) ) {
			wc_add_notice( __( 'This is a sample notice message', 'wc-donations' ), 'notice' );
		}

		if( WC_Donations_Helpers::get_sample_number( $product_id ) < 10 ){
			$passed = false;
			wc_add_notice( __( 'This is a sample error message', 'wc-donations' ), 'error' );
		}
